...
function buildPrompt(analysis: NormalizedVideoAnalysis, options?: QuizGenerationOptions, attemptNumber = 1) {
  ...
  return [
    ...
    '3. DENSITY: Aim for ~5 moments per 6 minutes.',
    '4. VERIFICATION: Ensure the "correct_option" is grounded in the cues immediately following the timestamp.',
    '5. TIMESTAMPING: Use the onset of the event, not a reaction shot or aftermath. The chosen timestamp must be at most 2 seconds earlier than the first visible/audio cue for that event.',
    ...
    'For each timestamp (whether new or existing), perform the following validation checks:',
    '1. FORWARD-PREDICTABILITY CHECK: Using only data from timestamps BEFORE this moment, is it already possible to tell what is going to happen?',
    '   - If YES: Try moving the timestamp EARLIER. Repeat the check until you find the earliest point where the outcome is still not obvious.',
    '   - If NO: Try moving the timestamp FORWARD and repeat the check. Find the LATEST timestamp where it is still NOT possible to tell what will happen.',
    '2. PREVIOUS EVENT CHECK: Check if the event at the previous timestamp is still happening at the current timestamp.',
    '   - If YES: Move the current timestamp FORWARD until the previous event is no longer happening.',
    '   - If this movement causes any of the issues mentioned above (forward-predictability problems), abandon this timestamp entirely and skip it.',
    '3. RESOLUTION: After all adjustments, the timestamp must point to a moment where the outcome is NOT yet predictable, and no previous events are still occurring.',
    'Preserve chronological order. If a moment cannot be resolved using these rules, reject it entirely.',
    '',
    '### DATA TO ANALYZE',
