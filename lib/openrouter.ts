...
function buildPrompt(analysis: NormalizedVideoAnalysis) {
  ...
  return [
    ...
    '- Before finalizing, validate EVERY timestamp against transcript/cues/context and keep it only if it clearly makes sense as a real, distinct prediction point.',
    '- Discard any timestamp that is ambiguous, not grounded in evidence, outside valid range, too close to another chosen moment, or not tied to a meaningful immediate next event.',
    '- If unsure about a timestamp, do not include it.',
